...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_sampling_intervals(self):
        ...
        for recording_id in self.recording_dict.keys():
            ...
            # print(sampling_domain)
            # print(type(self._get_data_object(recording_id)))
            # print(self._get_data_object(recording_id).__dict__)
            sampling_intervals = getattr(
                self._get_data_object(recording_id), sampling_domain
            )
            # print((sampling_intervals.start))
            # exit()
    ...
    def _get_session_id_with_prefix(self, data: Data) -> str:
        ...
        # return f"{self.session_id_prefix_fn(data)}{data.session.id[0]}"
    ...
    def _get_subject_id_with_prefix(self, data: Data) -> str:
        ...
        # return f"{self.subject_id_prefix_fn(data)}{data.subject.id[0]}"
